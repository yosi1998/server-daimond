...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.status(401).json({ msg: "האימייל אינו רשום במערכת" });
    }
    ...
    if (!passwordValid) {
      return res.status(401).json({ msg: "סיסמא שגויה" });
    }
    const token = createToken(user._id, user.role, user.userName);
    res.json({ token, role: user.role, userName: user.userName }); // החזר את userName כחלק מהתוצאה
...
router.patch("/updateCart", auth, async(req,res) => {
  try{
    if(!Array.isArray(req.body.cart_ar)){
      return res.status(400).json({msg:"You need to send cart_ar"});
    }
    const data = await UserModel.updateOne({_id:req.tokenData._id},{cart_ar:req.body.cart_ar})
    res.json(data);
  }
  catch(err){
    console.log(err);
...
router.patch("/updateFavs", auth, async(req,res) => {
  try{
    if(!Array.isArray(req.body.favs_ar)){
      return res.status(400).json({msg:"You need to send favs_ar"});
    }
